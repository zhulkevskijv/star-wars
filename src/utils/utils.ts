...
export const getPropertiesById = <Type>(
  propertyName: string,
  ...
  return allData
    .filter((value) =>
      stringData.some((stringValue) => (value as any).url == stringValue)
    )
    .map((value) => (value as any)[propertyName]);
};

export const comparePeople = (a: Person, b: Person): number =>
  +computeId(a.url) - +computeId(b.url);
