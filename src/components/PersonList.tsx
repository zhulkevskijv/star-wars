...
const PersonList = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    getAllPages('/people', (people: Person[]) =>
      setPeople((prevState) => [...prevState, ...people])
    ).finally(() => setIsLoading(false));
    getAllPages('/films', (films: Film[]) =>
      setFilms((prevState) => [...prevState, ...films])
    );
    getAllPages('/species', (species: Specimen[]) =>
      setSpecies((prevState) => [...prevState, ...species])
    );
    getAllPages('/starships', (starships: Specimen[]) =>
      setStarships((prevState) => [...prevState, ...starships])
    );
  }, []);
  ...
  useEffect(() => {
    ...
    if (filters.film) {
      ...
      setFilteredPeople((prevPeople) =>
        prevPeople.filter((pers) =>
          pers.films.some((film) => film.includes(filmId))
        )
      );
    }
    if (filters.specimen) {
      ...
      setFilteredPeople((prevPeople) =>
        prevPeople.filter((pers) =>
          pers.species.some((specimen) => specimen.includes(specimenId))
        )
      );
    }
    if (filters.birthYearTo) {
      setFilteredPeople((prevPeople) =>
        prevPeople.filter(
          (person) => computeYear(person.birth_year) <= filters.birthYearTo
        )
      );
    }
    if (filters.birthYearFrom) {
      setFilteredPeople((prevPeople) =>
        prevPeople.filter(
          (person) => computeYear(person.birth_year) >= filters.birthYearFrom
        )
      );
    }
  }, [filters, people]);
