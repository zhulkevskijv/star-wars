...
const FilterPane = ({ films, species, setFilters, filters } : FilterPaneProps) => {
  ...
  const [birthFromEra, setBirthFromEra] = useState(filters.birthYearFrom < 0 ? "BBY" : "ABY");
  const [birthToEra, setBirthToEra] = useState(filters.birthYearTo < 0 ? "BBY" : "ABY");

  useEffect(() => {
    setFilters({ birthYearFrom: birthFromEra === "BBY" ? -1*birthFrom : birthFrom });
  ...
  return <StyledCard>
    <Card.Body>
      ...
      <Row className="g-2">
        <Col md={8}>
          <Form.Control as="select" onChange={(event) => setBirthFrom(+event.target.value)}>
            {[...(Array(1000).keys())].sort((a : number, b : number) => b - a).map((year)=><option key={year} value={year}>{year}</option>)}
          </Form.Control>
        </Col>
        <Col md={4}>
          <Form.Control as="select" onChange={(event) => setBirthFromEra(event.target.value)}>
      ...
      <Row className="g-2">
        <Col md={8}>
          <Form.Control as="select" onChange={(event) => setBirthTo(+event.target.value)}>
            {[...Array(1000).keys()].sort((a : number, b : number) => b - a).map((year)=><option key={year} value={year}>{year}</option>)}
          </Form.Control>
        </Col>
        <Col md={4}>
          <Form.Control as="select" onChange={(event) => setBirthToEra(event.target.value)}>
            <option value={"ABY"}>ABY</option>
            <option value={"BBY"}>BBY</option>
          </Form.Control>
        </Col>
